...
getsortinoRatio_from_to <- function(symbols,from_trading_date, to_trading_date){
  sortino_ratio <- c()
...
us_sortino_start <- function(start_date,end_date,min_volume,max_volume,min_close, max_close,frequenct){
  ...
  filepath <- paste0("./us_sortinoratio/",start_date,"_",end_date,"_",min_close,"_",max_close,".csv" )
...
draw_recent_sortino <- function(start_date,end_date,min_volume,max_volume,min_close, max_close,frequenct){
  symbols <-getSymbols_period_volume_price_frequency(start_date,end_date,min_volume,max_volume,min_close, max_close,frequenct)
  ...
  # print (symbol_sortino_df)
  ggplot(symbol_sortino_df,aes(x=symbols,y=sortino_ratio))+geom_bar(stat="identity")
}
...
getPeriod_sortinoratio <- function(ticker, fromDate, toDate){
  ...
  for( i in (1: nrow(period_df))){
    ...
    symbol_sortinoratio <- c(symbol_sortinoratio,getsortinoRatio_from_to(ticker, period_df[i,1], period_df[i,2]))
  }
...
draw_line <- function (symbols, startDate, endDate){
  ...
  ggplot(data = sortinos, aes (x=endDate, y=sortino, color= symbol, group = symbol)) + 
    geom_line() + 
    facet_wrap(~ symbol)
...
us_sortino_start(args[1],args[2],args[3],args[4],args[5],args[6],args[7])